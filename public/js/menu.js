$(document).ready(function () {
  ...
  $menu = $(".menu_left__list .menu_left__list_element a").removeClass(
    "active"
  );
  ...
  $('.menu_left__list .menu_left__list_element a[href="' + url + '"]').addClass(
    "active"
  );
...
function initCookie() {
  if ($.cookie("cookie_accept") == null) {
    ...
    $(".menu_mobile__list_element_control").hide();
  }
}
...
function AccptCookie() {
  ...
  $(".menu_mobile__list_element_control").show();
  $.cookie("cookie_accept", "value", { expires: 700, path: "/" });
}
function HidePopup() {
  $(".cookies_wrapper").removeClass("active");
  $(".minimized").addClass("active");
}
function ShowPopup() {
  $(".cookies_wrapper").addClass("active");
  $(".minimized").removeClass("active");
}
