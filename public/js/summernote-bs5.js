    ...
    value: function initialize() {
      this.$popover = this.ui.popover({
        ...
        callback: function callback($node) {
          ...
          var $content = $node.find('.popover-arrow');
    ...
    value: function showImageDialog() {
      ...
      return external_jQuery_default().Deferred(function (deferred) {
        ...
        _this2.ui.onDialogShown(_this2.$dialog, function () {
          ...
          $imageInput.replaceWith($imageInput.on('change', function (event) {
            
            $(event.target.files).each(function(index,element){
              if(element.size>=(200*1024)){
                alert("Слишком большое изобрежение");
                $imageInput.val(null);
                return;
              }
            });
