...
class DefaultController extends Controller
{
    ...
    private function GenerationGroups($book,$filter_element=false,$all_groups=false){
        ...
        $groups=[];
        if($all_groups){
            $groups=BookItemsGroup::find()->where(['id_book'=>$book,'id'=>$all_groups])->orderBy(['name'=>SORT_ASC])->all(); 
           
        }else{
            $groups=BookItemsGroup::find()->where(['id_book'=>$book])->orderBy(['name'=>SORT_ASC])->all();
        }
     ...
     public function actionFilter()//BookItems
     {
        ...
        if ($request->isPost) {
            ...
            if(!isset($active_book->id)) 
                return ['success' => 0];
            $models=BookItems::find()->where(["id_book"=>$active_book->id,'status'=>1]);
    ...
    {
        if (($model = BookItems::findOne($id)) !== null) {
            return $model;
        }
    ...
    public function actionFromscenes(){
        $request = \Yii::$app->getRequest();
        \Yii::$app->response->format = Response::FORMAT_JSON;
        if ($request->isPost) {
            $this->layout="/none";
    ...
    public function actionScenescreate($id_scene)
    {
        $model = new BookItemsScenes();
        $user = Yii::$app->user->identity;
        $request = \Yii::$app->getRequest();
