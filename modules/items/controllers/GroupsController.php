...
class GroupsController extends Controller
{
     ...
     public function actionList(){
        ...
        if ($request->isPost) {
           \Yii::$app->response->format = Response::FORMAT_JSON;
            ...
            $result= BookItemsGroup::find()->where(["id_user"=>$user->id,'status'=>1,'id_book'=> $book_id])->all();
            return [
    ...
    private function findAndDelete($group_id){
        $elements=BookItems::find()->where(['status'=>1])->andWhere(['like', 'id_group', $group_id])->all();
    ...
    {
        if (($model = BookItemsGroup::findOne($id)) !== null) {
            return $model;
        }
