...
use app\modules\locations\models\BookLocationsGroup;

use yii\helpers\ArrayHelper;
use Yii;
/**
 * This is the model class for table "book_locations".
 *
 * @property int $id
 ...
 * @property Books $book
 */
class BookLocations extends \yii\db\ActiveRecord
{
    public $group;
    public $imageFile;
    /**
    ...
    public function upload()
    {
            ...
            $this->imageFile->saveAs(Yii::getAlias('@webroot').'/img/locations/' .$name) ;
            $this->imageFile="";
            return $name;
    }
    ...
    { 
        ...
        $groups = BookLocationsGroup::find()->where(["id_user"=>$user->id,'status'=>1,'id_book'=> $this->id_book])->all();
    ...
    public function findGroup(){
        ...
        if(!$this->group ||  (is_countable($this->group) && count($this->group)<=0)){
            ...
            $BookPersGroup=BookLocationsGroup::find()->where(['id_book'=>$this->id_book,'isDefault'=>1])->one();
            ...
            foreach($this->group as $group){
                if(intval($group)>0){
                    ...
                    if($group!="-" && $group!="0" && $char=="@"){
                        $color = array_rand(BookLocationsGroup::getColorList(), 1);
                        $BookGroups = new BookLocationsGroup();
    ...
    public function getGroupName(){
        ...
        return $result;
            foreach($this->id_group as $group){
                $group_res = BookLocationsGroup::find()->where(["id"=>$group])->one();
    ...
    public function getGroupColor(){
        if(!isset($this->id_group))
        return "transparent";
        $group = BookLocationsGroup::find()->where(["id"=>$this->id_group])->one();
    ...
    { 
        $user=Yii::$app->user->identity;
        $book_id=Books::find()->where(["id_user"=>$user->id,"status"=>1,'main'=>1])->one()->id;
        return BookLocationsGroup::find()->where(["id_user"=>$user->id,'status'=>1,'id_book'=> $book_id])->all();
    ...
    public function getMaxSort(){
        $user=Yii::$app->user->identity;
        $book_id=Books::find()->where(["id_user"=>$user->id,"status"=>1,'main'=>1])->one()->id;
        return BookLocations::find()->where(['id_book'=>$book_id])->max('sort')+1;
    }

    public function getGroup($group_id){
        return BookLocationsGroup::find()->where(["id"=>$group_id])->one();
    }
}
