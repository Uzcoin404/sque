...
class GroupsController extends Controller
{
     ...
     public function actionList(){
        ...
        //if ($request->isPost) {
           \Yii::$app->response->format = Response::FORMAT_JSON;
            ...
            $result= BookLocationsGroup::find()->where(["id_user"=>$user->id,'status'=>1,'id_book'=> $book_id])->all();
            return [
    ...
    private function findAndDelete($group_id){
        $elements=BookLocations::find()->where(['status'=>1])->andWhere(['like', 'id_group', $group_id])->all();
    ...
    {
        if (($model = BookLocationsGroup::findOne($id)) !== null) {
            return $model;
        }
