...
class DefaultController extends Controller
{
     ...
     private function GenerationGroups($book,$filter_element=false,$all_groups=false){
        ...
        $groups=[];
        if($all_groups){
            $groups=BookLocationsGroup::find()->where(['id_book'=>$book,'id'=>$all_groups])->orderBy(['name'=>SORT_ASC])->all(); 
        
        }else{
            $groups=BookLocationsGroup::find()->where(['id_book'=>$book])->orderBy(['name'=>SORT_ASC])->all();
        }
     ...
     public function actionFilter()
     {
        ...
        if ($request->isPost) {
            ...
            if(!isset($active_book->id)) 
                return ['success' => 0];
            $models=BookLocations::find()->where(["id_book"=>$active_book->id,'status'=>1]);
    ...
    {
        if (($model = BookLocations::findOne($id)) !== null) {
            return $model;
        }
