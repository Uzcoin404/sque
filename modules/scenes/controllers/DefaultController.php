...
class DefaultController extends Controller
{
     ...
     public function behaviors()
     {
         return [
             'access' => [
                 ...
                 'only' => ['index','filter','create','update','master','form','validate','delete','copy','active','sort','alllist','alllistadd','clearimg','updatetext'],
                 'rules' => [
                     [
                         'actions' => ['index','filter','create','update','master','form','validate','delete','copy','active','sort','alllist','alllistadd','clearimg','updatetext'],
                         'allow' => true,
                         'roles' => ['@'],
                     ],
                 ],
     ...
     private function GenerationGroups($book,$filter_element=false,$all_groups=false){
        ...
        $groups=[];
        if($all_groups){
            $groups=BookScenesGroup::find()->where(['id_book'=>$book,'id'=>$all_groups])->orderBy(['name'=>SORT_ASC])->all(); 
           
        }else{
            $groups=BookScenesGroup::find()->where(['id_book'=>$book])->orderBy(['name'=>SORT_ASC])->all();
        }
     ...
     public function actionFilter()//BookScenes
     {
        ...
        if ($request->isPost) {
            ...
            if(!isset($active_book->id)) 
                return ['success' => 0];
            $models=BookScenes::find()->where(["id_book"=>$active_book->id,'status'=>1]);

            /*** FILTERS ***/
            if(isset($value['filter_items_name'])){
                $models->andWhere(['like', 'name', $value['filter_items_name']]);
            }
    ...
    {
        if (($model = BookScenes::findOne($id)) !== null) {
            return $model;
        }
        ...
        \Yii::$app->response->format = Response::FORMAT_JSON;
            if ($request->isPost && $model->load($request->post())) {
                ...
                //$model->save();
                    ...
                    //if($model->save()){
                     //   return ['success' =>$model->id,'book_id'=>$model->id_book];
                   // }
    ...
    public function actionUpdatetext($id)
    {
        ...
        \Yii::$app->response->format = Response::FORMAT_JSON;
        $model =$this->findModel($id);
        $model_ajx =$this->findModel($id);
            if ($request->isPost && $model_ajx->load($request->post())) {
               
                $user = Yii::$app->user->identity;
                $model_ajx->imageFile = UploadedFile::getInstance($model, 'imageFile');

                if(isset($model_ajx->imageFile)){
                    $model_ajx->image=$model_ajx->upload();
                    $model_ajx->imageFile="";
                }
                $model_ajx->group=$model->group;
                $model_ajx->id_group=$model->id_group;
                $model_ajx->id_group=$model_ajx->findGroup();
                $reload_page=0;
        ...
        if ($request->isPost) {
            ...
            if($Books->save()){
                ...
                Notes::updateAll(
                    ...
                    "type=:type AND id_target =:id_target and id_user = :id_user",
                    [
                        ":type"=>2,
                        ":id_user"=>$user->id,
                        ":id_target"=>$Books->id,
    ...
    public function actionAlllist(){
        ...
        if ($request->isPost) {
           \Yii::$app->response->format = Response::FORMAT_JSON;
            ...
            $finds= BookPers::find()->where(["id_book"=>$active_book->id,'status'=>1])->all();
            FOREACH($finds as $r){
                $result[]=[
                    "name"=>"(пер.) ".$r->nickname." ".$r->name,
                    "id"=>$r->id,
                    "type"=>3
                ];
            }

            $finds= BookLocations::find()->where(["id_book"=>$active_book->id,'status'=>1])->all();
            FOREACH($finds as $r){
                $result[]=[
                    "name"=>"(лок.) ".$r->name,
                    "id"=>$r->id,
            ...
            $finds= BookItems::find()->where(["id_book"=>$active_book->id,'status'=>1])->all();
            FOREACH($finds as $r){
                $result[]=[
                    "name"=>"(пред.) ".$r->name,
                    "id"=>$r->id,
    ...
    public function actionAlllistadd(){
        $request = \Yii::$app->getRequest();
        if ($request->isPost) {
           \Yii::$app->response->format = Response::FORMAT_JSON;
            $value=$request->post();
            ...
            $user=Yii::$app->user->identity;
            if($value['type']==3){//Персонажи
                
                $find=BookPersScenes::find()->where(
                    [
                        "id_user"=>$user->id,
                        "id_scenes"=>$scene_id,
                        "id_pers"=>$id,
                ...
                if(isset($find->id)) return [1];
                $element= new BookPersScenes();
                $element->id_user=$user->id;
                $element->id_scenes=$scene_id;
                $element->id_pers=$id;
                if($element->save()){
                    return ["success"=>1];
                }
            }
            if($value['type']==4){//Локации
                
                $find=BookLocationsScenes::find()->where(
                    [
                        "id_user"=>$user->id,
                        "id_scenes"=>$scene_id,
                        "id_locations"=>$id,
                ...
                if(isset($find->id)) return [1];
                $element= new BookLocationsScenes();
                $element->id_user=$user->id;
                $element->id_scenes=$scene_id;
                $element->id_locations=$id;
                if($element->save()){
                    return ["success"=>1];
                }
            }
            if($value['type']==5){//Предметы
                $find=BookItemsScenes::find()->where(
                    [
                        "id_user"=>$user->id,
                        "id_scenes"=>$scene_id,
                        "id_items"=>$id,
